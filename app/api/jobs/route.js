...
export const POST = route(async ({ body, user, req }) => {
  ...
  if (!hasItems && !body.jobType) throw bad('Choose a job type');
  if (hasItems && body.items.some((i) => !i.jobType)) throw bad('Every item needs a job type');
  ...
  if (!customer) throw bad("Enter the customer's name");
  ...
  if (!built.items.length) throw bad('Add at least one item to the job');
  ...
  await logAction(user, 'job.create', {
    entity: 'Job',
    entityId: job._id,
    label: `Created ${status === 'quote' ? 'quote' : 'job'} ${job.jobNumber} for ${customer.name}`,
